 ...
 */
 ...
 //Floor and Ceiling
     ...
     */
     ...
     for(int w=0; w<7; w++)
       {
	 new G4PVPlacement(0, 
			   G4ThreeVector(-6*ft,0., targetPos+wallTotarget+4*ft*(w+3)), 
			   wallLV, 
			   "wallBlock",
			   worldLV, 
			   false, 
 ...
 fVolumesShiftedByCalorDist[calorIndex] = 
   new G4PVPlacement(0, 
		   G4ThreeVector(0., 0., fCalorDist+crystalLength/2+1.38*ft), 
		   pipeLV, 
		   "PipeLV", 
		   worldLV, 
		   false, 
 ...
 fVolumesShiftedByCalorDist[calorIndex] =new G4PVPlacement(0, 
		   G4ThreeVector(0.,0., fCalorDist+crystalLength/2+6.8*ft), 
			 beamdumpLV, 
		   "beamdump",
		   worldLV, 
		   false, 
 ...
 */
 //Omni detector
     ...
     G4VSolid* omni = 
       new G4Tubs("omniS", fCalorDist*std::tan(fThetaMin), fCalorDist*std::tan(fThetaMax),
		  .01*cm, 0.*deg, 360.*deg);
...
void DetectorConstruction::SetCalorDist(G4double distance)
{
  ...
  for (int i=0; i<1225; i++)
      ...
      if (!fPhysCalor[i])
	{
      ...
      fPhysCalor[i]->SetTranslation(G4ThreeVector(temp.getX(), temp.getY(), temp.getZ()-fCalorDist+distance));
  ...
  for(int i=0; i<3; i++)
    {
      G4ThreeVector temp = fVolumesShiftedByCalorDist[i]->GetTranslation();
      G4ThreeVector translation = G4ThreeVector(temp.getX(), temp.getY(), temp.getZ()-fCalorDist+distance);
      fVolumesShiftedByCalorDist[i]->SetTranslation(translation);

    }

  DetectorConstruction::PositionChambers(distance-.5*fTargetLength); 
...
void DetectorConstruction::PositionChambers(G4double distance)
{
  ...
  vector<G4LogicalVolume*> caps(fNchambers+1);
  vector<G4double> stageRad(fNchambers+1);
  vector<G4VPhysicalVolume*> physChambers2(fNchambers);
  vector<G4VPhysicalVolume*> physCaps2(fNchambers+1);
  ...
  for (G4int n=0; n<fNchambers; n++)
    {
      ...
      G4Tubs * capParam = new G4Tubs("cap", stageRad[n]+fChamThickness-.1*cm, stageRad[n+1]+fChamThickness-.1*cm, fCapThickness/2, 0.*deg, 
				     360.*deg);
      
      caps[n] = new G4LogicalVolume(capParam, 
				    fBeamLineMaterial, 
				    "capsLV", 
				    0, 0, 0);
      ...
      /*
      physCaps[n] = new G4PVPlacement(0, 
				      G4ThreeVector(0., 0., posCham+.5*nStageLength-.5*fCapThickness), 
				      caps[n], 
				      "CapsPV", 
				      worldLV, 
				      false, 
      ...
      G4Tubs * capParam = new G4Tubs("cap", 20.*cm, stageRad[fNchambers]-fChamThickness, fCapThickness/2, 0.*deg, 
				     360.*deg);
      
      caps[fNchambers] = new G4LogicalVolume(capParam, 
				    fBeamLineMaterial, 
				    "capsLV", 
				    0, 0, 0);
      /*
      physCaps[fNchambers] = new G4PVPlacement(0, 
				      G4ThreeVector(0., 0., firstSeg+(fNchambers)*nStageLength+ targetPos-.5*fCapThickness), 
				      caps[fNchambers], 
				      "CapsPV", 
				      worldLV, 
				      false, 
